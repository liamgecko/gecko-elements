...
export const archivedFormColumns: ColumnDef<Form>[] = [
  {
  ...
  {
    id: "archivedBy",
    accessorFn: (row) => row.archivedBy?.name ?? "",
    meta: { label: "Archived by" } satisfies DataTableColumnMeta,
    header: ({ column }) => (
      <DataTableColumnHeader column={column} title="Archived by" />
    ),
    sortingFn: (rowA, rowB) =>
      (rowA.original.archivedBy?.name ?? "").localeCompare(
        rowB.original.archivedBy?.name ?? "",
      ),
    cell: ({ row }) => {
      const { archivedBy } = row.original
      if (!archivedBy) return <span className="text-muted-foreground">—</span>

      return (
        <div className="flex min-w-0 items-center gap-3">
          <Avatar size="md">
            <AvatarFallback>{archivedBy.initials}</AvatarFallback>
          </Avatar>
          <DataTableMultiLineCell
            primary={archivedBy.name}
            secondary={formatFormDate(archivedBy.archivedAt)}
          />
        </div>
      )
    },
