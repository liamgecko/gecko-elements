...
function Toast({ className, ...props }: ToastPrimitive.Root.Props) {
  return (
    <ToastPrimitive.Root
      data-slot="toast"
      className={cn(
        "group/toast pointer-events-auto absolute right-0 bottom-0 z-[calc(1000-var(--toast-index))] w-full origin-bottom rounded-2xl border bg-popover text-popover-foreground shadow-lg outline-none select-none will-change-transform focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50",
  ...
  return toasts.map((toastItem) => (
    <Toast key={toastItem.id} toast={toastItem} data-type={toastItem.type}>
      <ToastContent>
        ...
        <ToastAction />
        {!toastItem.actionProps && <ToastClose />}
      </ToastContent>
    </Toast>
  ));
}
...
/* eslint-disable react-refresh/only-export-components -- Toast intentionally exports its shared manager and Base UI hooks with the compound components. */
export {
  Toaster,
  Toast,
  ToastAction,
