...
function SidebarProvider({
  ...
  const [_open, _setOpen] = React.useState(() => {
    if (!persistState || typeof document === "undefined") return defaultOpen;
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      if (persistState) {
        document.cookie = `${encodeURIComponent(storageKey)}=${openState}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}`;
      }
    },
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      className={cn(
        "hover:after:bg-sidebar-border absolute inset-y-0 z-20 hidden w-4 transition-all ease-linear motion-reduce:transition-none group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:start-1/2 after:w-[2px] sm:flex ltr:-translate-x-1/2 rtl:-translate-x-1/2",
...
function SidebarFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar="footer"
      className={cn(
        "gap-2 p-2 flex flex-col border-t border-sidebar-border",
        className,
      )}
      {...props}
    />
...
function SidebarGroupLabel({
  ...
  return useRender({
    ...
    props: mergeProps<"div">(
      {
        className: cn(
          "text-sidebar-foreground/70 ring-sidebar-ring h-8 rounded-md px-2 text-2xs font-medium transition-[margin,opacity] duration-200 ease-linear motion-reduce:transition-none group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0 focus-visible:ring-2 [&>svg]:size-4 [&>svg:first-of-type]:stroke-[1.75] flex shrink-0 items-center outline-hidden [&>svg]:shrink-0",
          className,
        ),
      },
      props,
  ...
  const walk = (value: React.ReactNode) => {
    React.Children.forEach(value, (child) => {
      ...
      if (child.type === DropdownMenuItem) {
        const props = child.props as React.ComponentProps<
...
function SidebarMenuBadge({
  ...
  return (
    <div
      ...
      className={cn(
        "text-sidebar-foreground peer-hover/menu-button:text-sidebar-accent-foreground peer-data-active/menu-button:text-sidebar-accent-foreground pointer-events-none absolute end-1 h-5 min-w-5 rounded-md px-1 text-2xs font-medium peer-data-[size=default]/menu-button:top-1.5 peer-data-[size=lg]/menu-button:top-2.5 peer-data-[size=sm]/menu-button:top-1 flex items-center justify-center tabular-nums select-none group-data-[collapsible=icon]:hidden",
        className,
      )}
      {...props}
    />
